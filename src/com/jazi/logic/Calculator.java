...
//  Обратите внимание, что каждым вторым элементом я добавляю " ". Для того, чтобы потом мне было легче работать со строкой
...
//
//              Конструкция 1: первым элементом, который мы отыскали были вторые "("
                ...
                if (strList.get(i).equals("(")) {
                    ...
                    String testRecursion = String.valueOf(eval(recursion.toString()));
...
//                Конструкция 2: первым элементом, который мы отыскали был  ")"
                ...
                if (strList.get(i).equals(")")) {
                    ...
                    String testRecursion2 = String.valueOf(eval(recursion2.toString()));
...
//        System.out.println();
        ...
        List<String> stringList2 = new ArrayList<>(Arrays.asList(expression.trim().split(" ")));
...
//  Если что в комментариях под постом немного объясню, если кто-нибудь дочитает до сюда и у него будет желание)
            if (stringList2.contains("/")) {
                ...
                Double left = Double.valueOf(stringList2.get(index - 1));
                Double right = Double.valueOf(stringList2.get(index + 1));
                if (right == 0.0 ){
                ...
                if (index == 0) {
                    ...
                    result = getaDouble(stringList2, lastIndex, lastIndex + 1, lastIndex - 1);
                }
                else {
                    result = Double.valueOf(stringList2.get(index - 1)) - Double.valueOf(stringList2.get(index + 1));
                    stringList2.add(index - 1, String.valueOf(result));
                ...
                int index = stringList2.indexOf("+");
                result = getaDouble(stringList2, index, index - 1, index + 1);
            }
    ...
    protected static Double getaDouble(List<String> stringList2, int lastIndex, int i2, int i3) {
        Double result;
        result = Double.valueOf(stringList2.get(i2)) + Double.valueOf(stringList2.get(i3));
